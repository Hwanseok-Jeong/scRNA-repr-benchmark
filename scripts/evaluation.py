...
def main():
    ...
    X_tsne = adata.obsm[f"X_tsne_{args.method}"]
    X_umap = adata.obsm[f"X_umap_{args.method}"]
    ...
    results = {}
    for emb_name, X_low in zip(["tsne", "umap"], [X_tsne, X_umap]):
        print(f"\n[*] Evaluating {emb_name.upper()}...")
